...
#
# 与源码构建路径 front/Dockerfile 并存、互不影响：那个用于离线灾备
# （scripts/fetch-sources.sh + docker-compose.prod.yml），本文件用于 docker-compose.release.yml。
#
# 构建上下文约定为 release/<tag>/front，其中必须存在 front.jar。
# JAVA_OPTS、EXPOSE 与 HEALTHCHECK 与 front/Dockerfile 保持一致，避免两种路径下运行时行为漂移。
