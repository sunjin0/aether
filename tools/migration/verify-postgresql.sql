...
WHERE table_schema = 'public'
  AND table_name IN ('sys_user', 'sys_dict', 'agent_document', 'agent_document_chunk')
ORDER BY table_name;

SELECT indexname, indexdef
FROM pg_indexes
WHERE schemaname = 'public'
  AND indexname IN ('idx_agent_document_chunk_embedding_cosine', 'idx_agent_document_chunk_knowledge_base')
ORDER BY indexname;
...
FROM sys_dict
UNION ALL
SELECT 'agent_document', COUNT(*), MIN(id), MAX(id), COUNT(*) FILTER (WHERE deleted)
FROM agent_document
UNION ALL
SELECT 'agent_document_chunk', COUNT(*), MIN(id), MAX(id), COUNT(*) FILTER (WHERE deleted)
FROM agent_document_chunk;

-- Verify the pgvector operator and HNSW index without retaining test data.
BEGIN;
INSERT INTO agent_document_chunk (
    id, knowledge_base_id, document_id, chunk_index, content, token_count, embedding,
    created_at, updated_at, sort_num, deleted, state
) VALUES
    (900000000000000001, 1, 1, 0, 'vector smoke test A', 4, array_fill(0::real, ARRAY[1536])::vector,
     0, 0, 0, FALSE, 0),
