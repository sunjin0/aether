...
import com.aether.agent.mapper.AgentWorkflowWebhookTriggerMapper;
import com.aether.agent.service.AgentWorkflowExecutionService;
import com.aether.agent.service.AgentWorkflowService;
import com.aether.agent.service.AgentWorkflowWebhookTriggerService;
...
public class AgentWorkflowWebhookTriggerServiceImpl
    ...
    public AgentWorkflowWebhookTriggerSecretVo create(AgentWorkflowWebhookTriggerDto dto) {
        ...
        trigger.setWorkflowId(dto.getWorkflowId()); trigger.setServiceAccountId(dto.getServiceAccountId());
        trigger.setName(dto.getName()); trigger.setBusinessType(dto.getBusinessType());
        trigger.setBusinessIdExpression(dto.getBusinessIdExpression()); trigger.setIdempotencyKeyExpression(dto.getIdempotencyKeyExpression());
        ...
        trigger.setSigningSecret(AesUtil.encrypt(secret)); trigger.setEnabled(true);
        save(trigger);
    ...
    public boolean setEnabled(String id, boolean enabled) {
        AgentWorkflowWebhookTrigger trigger = required(id); trigger.setEnabled(enabled);
    ...
    public AgentWorkflowInstance trigger(String id, String timestamp, String signature, String rawBody, Map<String, String> headers) {
        ...
        try {
            ...
            for (Map.Entry<String, String> entry : mapping.entrySet()) variables.put(entry.getKey(), resolve(entry.getValue(), body, headers));
            ...
            trigger.setLastTriggeredAt(System.currentTimeMillis()); trigger.setLastErrorMessage(null); updateById(trigger);
            return instance;
        } catch (RuntimeException ex) {
            trigger.setLastErrorMessage(StringUtils.abbreviate(ex.getMessage(), 2048)); updateById(trigger);
            throw ex;
        }
    }
    ...
    private Object resolve(String expression, Map<String, Object> body, Map<String, String> headers) {
        ...
        if (expression.startsWith("$header.")) {
            ...
            for (Map.Entry<String, String> header : headers.entrySet()) if (wanted.equalsIgnoreCase(header.getKey())) return header.getValue();
            return null;
        }
        return expression;
    }
    ...
    private AgentWorkflowWebhookTriggerSecretVo secretVo(AgentWorkflowWebhookTrigger trigger, String secret) {
        ...
        value.setId(trigger.getId()); value.setWebhookUrl("/api/agent/workflow/webhook/" + trigger.getId()); value.setSigningSecret(secret);
        return value;
    }
