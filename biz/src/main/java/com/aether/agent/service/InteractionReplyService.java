...
public class InteractionReplyService {
    ...
    public String buildAnsweredQuestionConfig(AgentMessage question, Map<String, Object> answer, Long answeredAt) {
        ...
        if ("group".equals(config.getString("type"))) {
            ...
            if (!(answersObj instanceof Map)) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.group.answers.required"));
            ...
            for (int i = 0; i < questions.size(); i++) {
                ...
                if (!(itemAnswer instanceof Map)) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.question.reply.required"));
    ...
    private String renderGroupAnswer(JSONObject config, Map<String, Object> answer) {
        Object raw = answer.get("answers"); if (!(raw instanceof Map)) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.group.answers.required"));
        JSONArray questions = config.getJSONArray("questions"); if (questions == null || questions.isEmpty()) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.configuration.invalid"));
    ...
    private String renderChoiceAnswer(JSONObject config, Map<String, Object> answer) {
        List<String> selected = selected(answer.get("selected")); if (selected.isEmpty()) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.choice.selected.required"));
        if (!Boolean.TRUE.equals(config.getBoolean("multiple")) && selected.size() > 1) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.choice.single.required"));
        List<String> labels = new ArrayList<>(); JSONArray options = config.getJSONArray("options");
        for (String value : selected) { JSONObject found = findOption(options, value); if (found == null) { if (!Boolean.TRUE.equals(config.getBoolean("allowCustomInput")) || StringUtils.isBlank(value) || value.length() > 200) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.choice.selection.invalid")); labels.add(value); } else labels.add(found.getString("label") + "(" + found.getString("value") + ")"); }
    ...
    private boolean confirmed(Map<String, Object> answer) { Object value = answer.get("confirmed"); if (!(value instanceof Boolean)) throw new ServerException(400, I18nUtils.getMessage("agent.interaction.confirmation.required")); return Boolean.TRUE.equals(value); }
