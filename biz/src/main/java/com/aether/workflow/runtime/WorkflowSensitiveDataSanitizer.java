...
public class WorkflowSensitiveDataSanitizer {
    ...
    public WorkflowSensitiveDataSanitizer(@Value("${aether.workflow.security.mask-fields:password,secret,token,authorization,access_token,refresh_token}") String fields) {
        ...
        for (String field : fields.split(",")) if (!field.trim().isEmpty()) result.add(field.trim().toLowerCase(Locale.ROOT));
        this.sensitiveFields = result;
    }

    public String sanitizeJson(String text) {
        if (text == null || text.isEmpty()) return text;
        try { return JSON.toJSONString(sanitize(JSON.parse(text))); }
    ...
    private Object sanitize(Object value) {
        ...
        if (value instanceof Collection) {
            List<Object> result = new ArrayList<Object>(); for (Object item : (Collection<?>) value) result.add(sanitize(item)); return result;
        }
        return value;
    }
}
