...
| [持续智能体会话记忆与动态执行重构](agent/持续智能体会话记忆与动态执行重构方案.md) | Deep Agent 从单次运行升级为持续 Session、分层记忆和动态任务执行的完整方案。 |
| [持续智能体会话记忆与动态执行实施跟踪](agent/持续智能体会话记忆与动态执行实施跟踪.md) | 分阶段交付状态、跨仓库边界及发布前验收清单。 |
