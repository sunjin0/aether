...
const header = { fill: '#1F4D78', font: { bold: true, color: '#FFFFFF' }, horizontalAlignment: 'center', verticalAlignment: 'center' };
...
summary.getRange('A1:H1').format = { fill: '#1F4D78', font: { bold: true, color: '#FFFFFF', size: 16 }, horizontalAlignment: 'center', verticalAlignment: 'center' };
...
summary.getRange('D3:H3').merge(); summary.getRange('D3').values = [['Management metrics']]; summary.getRange('D3:H3').format = header;
...
changes.getRange('A1:I6').format.wrapText = true; changes.getRange('A:I').format.columnWidth = 18; changes.getRange('A1:I6').format.autofitRows();
changes.getRange('H2:H100').dataValidation = { rule: { type: 'list', values: ['Draft','Pending','Approved','Implemented','Rolled back'] } };
...
risks.getRange('A1:H7').format.borders = { preset: 'all', style: 'thin', color: '#D9E2F0' }; risks.getRange('A:H').format.columnWidth = 20; risks.getRange('A1:H7').format.wrapText = true; risks.getRange('A1:H7').format.autofitRows();
risks.getRange('D2:D100').dataValidation = { rule: { type: 'list', values: ['Low','Medium','High'] } }; risks.getRange('E2:E100').dataValidation = { rule: { type: 'list', values: ['Low','Medium','High'] } }; risks.getRange('H2:H100').dataValidation = { rule: { type: 'list', values: ['Open','Planned','In progress','Accepted','Closed'] } };

sla.getRange('A1:F1').values = [['Support plan','P1 response','P2 response','Coverage','Communication','Escalation owner']]; sla.getRange('A1:F1').format = header;
...
sla.getRange('A1:F4').format.borders = { preset: 'all', style: 'thin', color: '#D9E2F0' }; sla.getRange('A:F').format.columnWidth = 22; sla.getRange('A1:F4').format.wrapText = true; sla.getRange('A1:F4').format.autofitRows();
for (const sheet of [summary, changes, risks, sla]) { sheet.freezePanes.freezeRows(1); sheet.showGridLines = false; }
