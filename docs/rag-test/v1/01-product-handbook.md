...
Standard 套餐最多支持 50 名坐席、3 个工单队列和 90 天数据留存。Professional 套餐最多支持 300 名坐席、20 个工单队列和 365
天数据留存。Enterprise 套餐不限制坐席数量，默认数据留存为 1095 天，可通过合同延长。

## 重要版本规则

从 v3.2 起，知识库命中率统计口径改为“被用户采纳的答案数 / 已触发知识推荐的工单数”。v3.1 及更早版本使用“推荐答案展示次数 /
工单总数”，该口径已废弃。
