...
- 仅已启用且显式绑定的工具会进入 Agent 的模型工具作用域；平台内置工具同样需要绑定。
- MCP 工具调用默认先生成确认卡片；会话或工作流可按审批策略自动放行低风险或全部已授权调用。
...
工作流将复杂流程建模为有向图，支持人工录入/审批、工具、Agent、规则、转换、HTTP、通知、延时、事件等待、子流程及并行汇聚等节点，并提供发布版本、实例运行时、失败重试/回放/终止与终态业务回调：

- 触发方式：手动启动、业务系统（服务账号 + 幂等键）、Webhook（签名事件）、定时（Cron）。
- 运行时：持久化任务队列 + 租约领取，支持并发上限、执行期限（deadline）、数据脱敏与保留期清理。
- 回调：终态后向业务系统投递可验签、可重试的结果回调。
- 运营：实例指标、死信、人工重投。
- 子流程：固定已发布版本运行；父实例等待期间可展示最深层子流程的交互或审批，子流程异常或超时会回传并使父节点收敛失败。
...
- 普通聊天工具按会话确认策略执行；工作流工具节点使用 `ask`、`risky` 或 `never` 策略，未知工具或参数按高风险处理。定时触发实例仍自动批准其已配置工具节点。工具调用与结果均写入审计记录。
