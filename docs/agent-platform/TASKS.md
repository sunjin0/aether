...
**验收标准**：
- [x] 可通过管理后台配置 Agent 后，调用聊天接口获得模型回复
...
- [x] 设计工具执行器接口（`ToolExecutor`）
- [x] 实现 HTTP 工具执行器（`HttpToolExecutor`）
- [x] 支持超时、重试、限流
- [x] 支持请求头模板和请求体模板渲染
- [x] 支持响应提取规则（JSONPath 或正则）

### 6.2 安全策略

- [x] 实现 URL 白名单/黑名单校验
- [x] 实现请求方法限制（GET/POST）
- [x] 实现请求头过滤（禁止 `Authorization` 等敏感头）
- [x] 实现响应大小限制
- [x] 实现执行超时控制
- [x] 安全拦截记录审计日志
...
- [x] 扩展 `AgentChatService` 支持工具调用：
  [x]- 模型返回 `tool_calls` 时，解析工具调用请求
  [x]- 执行工具调用，获取结果
  [x]- 将工具结果返回给模型继续生成
  [x]- 持久化工具调用消息和运行记录
- [x] 工具调用日志写入 `agent_tool_call_log`
...
- [x] 实现会话生命周期查询
- [x] 实现消息统计（按会话、按 Agent）
- [ ] 实现会话活跃度分析（预留）

### 7.3 权限细化

- [x] 定义 Agent 管理权限路径（`/agent/definition`）
- [x] 定义模型供应商配置权限路径（`/agent/model-provider`）
- [x] 定义工具管理权限路径（`/agent/tool`）
- [x] 定义会话查看权限（仅本人）
- [x] 在 Controller 方法上添加 `@Permission` 注解
- [x] 测试权限拦截
