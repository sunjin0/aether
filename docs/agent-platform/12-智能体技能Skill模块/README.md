...
- Skill 支持上传 Markdown 与模板文件，作为版本化资源管理；已发布资源使用不可覆盖的对象存储键冻结内容。
- 文件生成由平台通用工具提供，不由 Skill 的资源、配置或版本选择执行入口。
- 工具调用、知识库访问仍遵循最小授权；Skill 只收窄、不扩权。

### 1.4 非目标

- 不提供通用远程代码执行能力；Skill 不包含可执行脚本入口。
...
- Markdown 仅注入已提取的纯文本；模板只注入白名单无逻辑模板的受限摘要。历史脚本资源不注入模型，也不会被执行。
...
- `generate_artifact` 仅接收已授权 Agent 运行的文本、结构化文档计划和目标格式。
- 不接受 Skill 编码、资源 ID、脚本、命令、镜像、挂载或网络参数。
- 平台统一维护渲染器、输出限额、审批、委派令牌与审计记录。
