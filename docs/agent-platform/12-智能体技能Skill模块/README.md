...
> 更新日期：2026-08-07
> 状态：已实现（数据库 V38+ 六表、Skill CRUD / 草稿 / 版本 / 发布、工具与知识库绑定、Agent-Skill 安装绑定、资源上传/列表/删除（对象存储）、提示词合成与工具收敛已接入 AgentChat、前端资源管理与提示词预览 UI、后端 Skill 单测）。运行期 `agent_run.skill_snapshot` 快照与提示词预算强制仍按本方案待跟进。
