...
> 更新日期：2026-08-08
> 状态：已实现（数据库 V38+ 六表、Skill CRUD / 草稿 / 版本 / 发布、工具与知识库绑定、Agent-Skill 安装绑定、资源上传/列表/删除（对象存储）、提示词合成与工具收敛已接入 AgentChat、前端资源管理与提示词预览 UI、后端 Skill 单测）。运行期 `agent_run.skill_snapshot` 已冻结 Skill、最小授权范围、脱敏输入和预算明细；已安装 Skill 的完整上下文超出模型输入预算时会在调用前拒绝，不会静默截断 Skill 指令。
...
- 脚本一期**只作为受版本控制的资源**供模型参考，不自动执行；受控执行能力放入二期。
- 工具调用、知识库访问仍遵循最小授权；Skill 只收窄、不扩权。

### 1.4 非目标

- 不直接执行上传脚本，不提供通用远程代码执行能力。
...
上传时校验扩展名与 MIME、单文件/单版本总大小和资源数量；拒绝压缩包及不在白名单内的类型。发布事务中复核对象 SHA-256 后冻结对象键。脚本一期仅存为版本化资源，不执行，默认不将完整脚本内容注入模型。
...
3. 逐 Skill 校验：
    ...
    - 资源对象存在、哈希匹配、状态启用；任一冻结资源异常则**请求前拒绝**，不回退到其他内容。
...
| 脚本文件 | 仅上传、版本化、保存哈希与说明，模型仅获得说明与哈希，**不执行** | 沙箱 MCP 工具执行 |
...
| 4. 二期增强 | Skill 市场/分类推荐、版本 diff、脚本沙箱执行、结构化输出强校验、Skill 指标与导入导出 |
