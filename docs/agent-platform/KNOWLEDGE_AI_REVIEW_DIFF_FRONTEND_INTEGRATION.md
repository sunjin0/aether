...
单条或批量“采纳”仅更新问题处理状态，不会修改文档正文。成功后应重新请求 Diff，使 `proposedContent`、已接受数量和问题状态保持一致。只有“应用已接受修改”成功后才会写入草稿；版本保持 `AI_REVIEWED`，前端应更新 checksum 并允许继续处理剩余问题或手动编辑正文。
...
1. 将一次性写入全部已接受建议。
2. 写入后文档版本保持 `AI_REVIEWED`。
3. 系统不会自动发起或要求新的 AI 审查。

成功时，响应中的 `contentChecksum` 和 `reviewStatus` 是更新后的预检版本状态，`requiresAiReview` 为 `false`。前端应更新本地 checksum，重新加载版本和 Diff；作者可继续处理问题或编辑正文。若返回 `409`，重新加载 Diff，不得静默重试。
...
stateDiagram-v2
  ...
  文档草稿已更新 --> 审查完成: reviewStatus=AI_REVIEWED
```

业务上，采纳建议只会暂存问题状态，用户可以继续处理当前审查中的其他建议。统一应用会变更版本正文，但不会结束预检会话；前端必须以刷新后的审查状态和 checksum 为准。AI 必审时，全部问题不再为 `pending` 后才允许提交。
...
5. 接入“应用已接受修改”，应用成功后更新预检状态和编辑 checksum。
6. 完成 409 刷新、手动修复问题标记及端到端验收。

验收用例：

- 审查成功时，左右两栏准确显示 `originalContent` 与 `proposedContent`。
- 点击问题可定位原文；删除类建议不会因右侧行号为 0 而报错。
- 应用建议后页面刷新，版本保持 `AI_REVIEWED`，并可继续处理剩余问题或手动编辑。
