...
This is a Java 8, Spring Boot 2.7.18, multi-module Maven project. There is no Maven wrapper, so commands assume `mvn` is installed and are run from the repository root.

Module dependency direction:

- `common`: shared infrastructure and utilities: response wrapper, base entity, i18n, exception handling, interceptor, Redis config, MyBatis-Plus config, permission AOP, token/AES utilities, validation helpers.
- `api`: contract/data layer. Contains MyBatis-Plus entities, VOs, mapper interfaces, service interfaces, and i18n/resource YAML files.
- `biz`: business implementation layer. Service implementations live under `biz/src/main/java/com/aether/**/service/impl` and generally extend MyBatis-Plus `ServiceImpl<Mapper, Entity>` while implementing interfaces from `api`.
- `admin`: executable Spring Boot admin/API application. Contains the discovered REST controllers and `AdminApplication`.
- `front`: executable Spring Boot application shell with `FrontApplication`; at the time this file was written it has no discovered controllers and reuses `biz/common`.
...
Existing tests are Spring Boot context tests under `admin/src/test/java` and `front/src/test/java`; they may load dev-profile configuration and expect local services/configuration to be available.

### Run applications locally

The POMs do not configure `spring-boot-maven-plugin`, so prefer direct plugin invocation instead of assuming the packaged jars are executable Spring Boot fat jars.
