...
public interface KnowledgeDocumentChunkMapper extends BaseMapper<KnowledgeDocumentChunk> {
     ...
     */
    @Select("<script>SELECT chunk.id, chunk.created_by, chunk.knowledge_base_id, chunk.document_id, chunk.document_version_id, chunk.chunk_index, chunk.content, chunk.token_count, " +
            ...
            "<if test='accountId != null and accountId != \"\"'>AND (chunk.created_by = #{accountId} OR chunk.created_by IS NULL) </if>" +
     ...
     */
    @Select("SELECT id, created_by, knowledge_base_id, document_id, document_version_id, chunk_index, content, token_count, " +
            ...
            "<if test='accountId != null and accountId != \"\"'>AND (created_by = #{accountId} OR created_by IS NULL) </if>" +
    ...
    @Insert("INSERT INTO knowledge_document_chunk " +
            "(id, created_by, updated_by, knowledge_base_id, document_id, document_version_id, chunk_index, content, token_count, embedding, page_no, section_path, content_hash, metadata, reference_count, last_referenced_at, created_at, updated_at, sort_num, deleted, state) " +
            "VALUES " +
            "(#{chunk.id}, (SELECT created_by FROM knowledge_document WHERE id = #{chunk.documentId}), (SELECT created_by FROM knowledge_document WHERE id = #{chunk.documentId}), #{chunk.knowledgeBaseId}, #{chunk.documentId}, #{chunk.documentVersionId}, #{chunk.chunkIndex}, #{chunk.content}, #{chunk.tokenCount}, " +
