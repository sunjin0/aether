...
public interface AgentToolRoutingIndexMapper extends BaseMapper<AgentToolRoutingIndex> {
     ...
     * 处理insertVector索引。
     */
    @Insert("INSERT INTO agent_tool_routing_index (id, created_by, updated_by, tool_id, content_hash, embedding_provider_id, embedding_model, embedding, index_status, failure_reason, indexed_at, created_at, updated_at, sort_num, deleted, state) " +
            "VALUES (#{item.id}, (SELECT created_by FROM agent_tool WHERE id = #{item.toolId}), (SELECT created_by FROM agent_tool WHERE id = #{item.toolId}), #{item.toolId}, #{item.contentHash}, #{item.embeddingProviderId}, #{item.embeddingModel}, CAST(#{item.embedding} AS vector), #{item.indexStatus}, #{item.failureReason}, #{item.indexedAt}, #{item.createdAt}, #{item.updatedAt}, #{item.sortNum}, #{item.deleted}, #{item.state})")
    int insertVectorIndex(@Param("item") AgentToolRoutingIndex item);

    /**
     * 更新Vector索引。
     */
    @Update("UPDATE agent_tool_routing_index SET updated_by=COALESCE(NULLIF(#{item.updatedBy}, ''), updated_by), content_hash=#{item.contentHash}, embedding_provider_id=#{item.embeddingProviderId}, embedding_model=#{item.embeddingModel}, embedding=CAST(#{item.embedding} AS vector), index_status=#{item.indexStatus}, failure_reason=#{item.failureReason}, indexed_at=#{item.indexedAt}, updated_at=#{item.updatedAt} WHERE id=#{item.id}")
