...
CREATE TABLE IF NOT EXISTS sandbox_execution_template_version (
    ...
    published_by VARCHAR(32), published_at BIGINT,
    created_at BIGINT, updated_at BIGINT, sort_num INTEGER NOT NULL DEFAULT 0,
    deleted BOOLEAN NOT NULL DEFAULT FALSE, state INTEGER NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS sandbox_execution_task (
    ...
    agent_definition_id VARCHAR(32), run_id VARCHAR(64), message_id VARCHAR(32),
    ...
    claimed_by VARCHAR(128), claimed_at BIGINT, lease_expires_at BIGINT,
    cancel_requested_at BIGINT, started_at BIGINT, completed_at BIGINT, expires_at BIGINT NOT NULL,
    failure_code VARCHAR(64), failure_reason TEXT, log_summary TEXT,
    created_at BIGINT, updated_at BIGINT, sort_num INTEGER NOT NULL DEFAULT 0,
    deleted BOOLEAN NOT NULL DEFAULT FALSE, state INTEGER NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS sandbox_execution_approval (
    id VARCHAR(32) PRIMARY KEY, task_id VARCHAR(32) NOT NULL,
    decision VARCHAR(16) NOT NULL, approver_user_id VARCHAR(32) NOT NULL,
    reason VARCHAR(1024), decided_at BIGINT NOT NULL,
    created_at BIGINT, updated_at BIGINT, sort_num INTEGER NOT NULL DEFAULT 0,
    deleted BOOLEAN NOT NULL DEFAULT FALSE, state INTEGER NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS sandbox_execution_event (
    id VARCHAR(32) PRIMARY KEY, task_id VARCHAR(32) NOT NULL,
    sequence BIGINT NOT NULL, event_type VARCHAR(32) NOT NULL, status VARCHAR(24),
    progress INTEGER, summary TEXT, occurred_at BIGINT NOT NULL,
    created_at BIGINT, updated_at BIGINT, sort_num INTEGER NOT NULL DEFAULT 0,
    deleted BOOLEAN NOT NULL DEFAULT FALSE, state INTEGER NOT NULL DEFAULT 0
);
...
VALUES
    ('sandbox_generic_document', 'generic-document', '通用文档生成', '固定办公运行时的 DOCX/XLSX/PDF 生成', TRUE, 'LOW',
     (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, 0, FALSE, 0)
...
VALUES
    ('sandbox_generic_document_v1', 'sandbox_generic_document', 1, TRUE,
     ...
     (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, 0, FALSE, 0)
ON CONFLICT DO NOTHING;
UPDATE sandbox_execution_template SET current_version_id = 'sandbox_generic_document_v1'
WHERE id = 'sandbox_generic_document' AND current_version_id IS NULL;
