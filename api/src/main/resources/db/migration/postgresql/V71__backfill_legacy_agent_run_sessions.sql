...
INSERT INTO agent_session (
    id, conversation_id, agent_definition_id, user_id, status, graph_thread_id,
...
SELECT DISTINCT ON (run.conversation_id)
    ...
    0, FALSE, 0
FROM agent_run run
JOIN agent_conversation conversation ON conversation.id = run.conversation_id AND conversation.deleted = FALSE
...
ORDER BY run.conversation_id, run.created_at DESC NULLS LAST, run.id DESC
...
INSERT INTO agent_task (
    id, session_id, user_id, agent_definition_id, title, status, current_run_id,
    created_at, updated_at, sort_num, deleted, state
)
SELECT
    md5('legacy-task:' || run.id),
    run.session_id,
    run.user_id,
    run.agent_definition_id,
    '历史运行 ' || left(run.id, 8),
    ...
    END,
    run.id,
    run.created_at, run.created_at, 0, FALSE, 0
FROM agent_run run
...
INSERT INTO agent_task_event (
    id, task_id, run_id, event_type, summary, occurred_at,
    created_at, updated_at, sort_num, deleted, state
)
SELECT
    md5('legacy-task-event:' || run.id),
    run.task_id,
    run.id,
    'task.migrated',
    '由历史 Agent 运行记录迁移生成',
    COALESCE(run.updated_at, run.created_at, 0),
    run.created_at, run.updated_at, 0, FALSE, 0
FROM agent_run run
