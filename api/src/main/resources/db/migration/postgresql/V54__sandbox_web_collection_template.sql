...
VALUES ('sandbox_web_collection', 'web-collection', '网页采集', 'HTTPS 白名单网页采集（需受控 egress 代理）', FALSE,
        'HIGH', 'sandbox_web_collection_v1',
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT, 40, FALSE, 0)
ON CONFLICT
    (id)
    DO NOTHING;
