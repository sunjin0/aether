...
INSERT INTO sys_resource (id, name, name_cn, path, type, icon, parent_id, leaf, description, state, deleted, created_at,
                          updated_at, sort_num)
VALUES ('menu_workflow', 'AI Workflows', 'AI 工作流', '/workflow', 'Resource_Type_Route', 'ApartmentOutlined', '0',
        FALSE,
        'Design, operate, and govern AI workflows / 编排、运行与治理 AI 工作流', 0, FALSE,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT, 21)
ON CONFLICT
    (id)
    DO UPDATE SET name = EXCLUDED.name, name_cn = EXCLUDED.name_cn, path = EXCLUDED.path,
...
SELECT md5('root:' || root_role.id || ':menu_workflow'),
       root_role.id,
       'menu_workflow',
       0,
       FALSE,
...
WHERE NOT EXISTS (SELECT 1
                  ...
                  WHERE x.role_id = root_role.id AND x.resource_id = 'menu_workflow' AND x.deleted = FALSE);
