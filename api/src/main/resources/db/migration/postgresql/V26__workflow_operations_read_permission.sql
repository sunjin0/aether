...
VALUES ('perm_wf_ops_read', 'Read', '可读', NULL, 'Resource_Type_Permission', NULL,
        'agent_workflow_operations', TRUE, 'View workflow operational metrics and dead letters / 查看工作流运营指标与死信', 0, FALSE,
...
WITH authorized_roles AS (
    SELECT DISTINCT role_id FROM sys_role_resource
    WHERE resource_id = 'agent_workflow_operations' AND deleted = FALSE
)
INSERT INTO sys_role_resource (id, role_id, resource_id, state, deleted, created_at, updated_at, sort_num)
SELECT md5('workflow-operations-read:' || authorized_roles.role_id), authorized_roles.role_id,
       'perm_wf_ops_read', 0, FALSE,
       (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, 1
