...
SELECT md5('root:' || root_role.id || ':perm_dashboard_read'), root_role.id, 'perm_dashboard_read', 0, FALSE,
       (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT,
       (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, 1
FROM root_role
WHERE NOT EXISTS (
    SELECT 1 FROM sys_role_resource x
    WHERE x.role_id = root_role.id AND x.resource_id = 'perm_dashboard_read' AND x.deleted = FALSE
);
