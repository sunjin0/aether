...
INSERT INTO sys_resource (id, name, name_cn, path, type, icon, parent_id, leaf, description, state, deleted, created_at,
                          updated_at, sort_num)
VALUES ('agent_skill', 'Agent Skills', '智能体技能', '/agent/skill', 'Resource_Type_Route', NULL, 'menu_agent', TRUE,
        'Manage versioned agent skill packages / 管理版本化智能体技能包', 0, FALSE,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT, 11),
       ('perm_agent_skill_read', 'Read', '可读', NULL, 'Resource_Type_Permission', NULL, 'agent_skill', TRUE,
        'View agent skills and versions / 查看智能体技能与版本', 0, FALSE,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT, 1),
       ('perm_agent_skill_write', 'Write', '可写', NULL, 'Resource_Type_Permission', NULL, 'agent_skill', TRUE,
        'Edit and publish agent skills / 编辑和发布智能体技能', 0, FALSE,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
...
WITH root_role AS (SELECT id FROM sys_role WHERE name = 'root' AND deleted = FALSE ORDER BY created_at LIMIT 1),
     resource_ids AS (SELECT id
                      FROM sys_resource
                      WHERE id IN ('agent_skill', 'perm_agent_skill_read', 'perm_agent_skill_write'))
INSERT
