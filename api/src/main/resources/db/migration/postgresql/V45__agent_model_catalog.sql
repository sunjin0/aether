...
ALTER TABLE agent_definition
    ADD COLUMN IF NOT EXISTS model_id VARCHAR (32);
ALTER TABLE knowledge_base
    ADD COLUMN IF NOT EXISTS embedding_model_id VARCHAR (32);
INSERT INTO agent_model_catalog (id, provider_id, name, capabilities, context_window, status, remark, created_at,
                                 updated_at, sort_num, deleted, state)
SELECT substr(md5('catalog:' || id), 1, 32),
       id,
       default_model,
       CASE WHEN lower(default_model) LIKE '%embedding%' THEN 'EMBEDDING' ELSE 'UNCONFIRMED' END,
       context_window,
       status,
       'Migrated from provider default model',
       created_at,
       updated_at,
       sort_num,
       deleted,
