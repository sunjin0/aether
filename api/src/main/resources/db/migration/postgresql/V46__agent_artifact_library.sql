...
ALTER TABLE agent_artifact
    ADD COLUMN IF NOT EXISTS agent_definition_id VARCHAR (32);
ALTER TABLE agent_artifact
    ADD COLUMN IF NOT EXISTS recycled_at BIGINT;

UPDATE agent_artifact artifact
SET user_id             = execution.user_id,
    agent_definition_id = execution.agent_definition_id FROM agent_sandbox_execution execution
WHERE artifact.execution_id = execution.id
  AND (artifact.user_id IS NULL
   OR artifact.agent_definition_id IS NULL);
...
INSERT INTO sys_resource (id, name, name_cn, path, type, icon, parent_id, leaf, description, state, deleted, created_at,
                          updated_at, sort_num)
VALUES ('agent_artifact', 'Generated Files', '生成文件', '/agent/artifact', 'Resource_Type_Route', NULL, 'menu_agent',
        TRUE, 'View and manage AI-generated files / 查看和管理 AI 生成文件', 0, FALSE,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT, 12),
       ('perm_agent_artifact_read', 'Read', '可读', NULL, 'Resource_Type_Permission', NULL, 'agent_artifact', TRUE,
        'View own AI-generated files / 查看本人 AI 生成文件', 0, FALSE,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT, 1),
       ('perm_agent_artifact_write', 'Write', '可写', NULL, 'Resource_Type_Permission', NULL, 'agent_artifact', TRUE,
        'Recycle and restore own AI-generated files / 删除和恢复本人 AI 生成文件', 0, FALSE,
        (EXTRACT(EPOCH FROM clock_timestamp()) * 1000):: BIGINT,
...
WITH root_role AS (SELECT id FROM sys_role WHERE name = 'root' AND deleted = FALSE ORDER BY created_at LIMIT 1),
     resource_ids AS (SELECT id
                      FROM sys_resource
                      WHERE id IN ('agent_artifact', 'perm_agent_artifact_read', 'perm_agent_artifact_write'))
