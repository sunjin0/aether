...
VALUES
 ('sandbox_python_check', 'python-lint-test', 'Python 检查与测试', '固定 Python lint/test 命令，输入为受控 ZIP 快照', FALSE, 'HIGH', 'sandbox_python_check_v1', (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, 30, FALSE, 0),
 ('sandbox_node_check', 'node-lint-test-build', 'Node 检查、测试与构建', '固定 Node lint/test/build 命令，输入为受控 ZIP 快照', FALSE, 'HIGH', 'sandbox_node_check_v1', (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT, 31, FALSE, 0)
...
VALUES
 ('sandbox_python_check_v1', 'sandbox_python_check', 1, TRUE,
