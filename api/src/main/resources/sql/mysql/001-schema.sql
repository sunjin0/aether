...
CREATE TABLE IF NOT EXISTS `knowledge_ai_review` (`id` BIGINT NOT NULL PRIMARY KEY, `knowledge_base_id` BIGINT NOT NULL, `document_id` BIGINT NOT NULL, `document_version_id` BIGINT NOT NULL, `source_checksum` VARCHAR(128) NOT NULL, `source_content` LONGTEXT, `model_provider_id` BIGINT, `model` VARCHAR(128), `prompt_version` VARCHAR(32), `status` VARCHAR(16) NOT NULL, `score` INT, `summary` LONGTEXT, `issues` LONGTEXT, `statistics` LONGTEXT, `error_message` LONGTEXT, `started_at` BIGINT, `finished_at` BIGINT, `created_at` BIGINT, `updated_at` BIGINT, `sort_num` INT NOT NULL DEFAULT 0, `deleted` TINYINT NOT NULL DEFAULT 0, `state` INT NOT NULL DEFAULT 0, KEY `idx_ai_review_version` (`document_version_id`,`created_at`,`deleted`));
CREATE TABLE IF NOT EXISTS `knowledge_ai_review_issue` (`id` BIGINT NOT NULL PRIMARY KEY, `ai_review_id` BIGINT NOT NULL, `document_version_id` BIGINT NOT NULL, `block_id` VARCHAR(64), `issue_type` VARCHAR(32) NOT NULL, `severity` VARCHAR(16) NOT NULL, `message` LONGTEXT NOT NULL, `original_excerpt` LONGTEXT, `suggested_patch` LONGTEXT, `handle_status` VARCHAR(16) NOT NULL DEFAULT 'pending', `handled_by` BIGINT, `handled_at` BIGINT, `handle_comment` LONGTEXT, `applied_content` LONGTEXT, `applied_checksum` VARCHAR(128), `created_at` BIGINT, `updated_at` BIGINT, `sort_num` INT NOT NULL DEFAULT 0, `deleted` TINYINT NOT NULL DEFAULT 0, `state` INT NOT NULL DEFAULT 0);
