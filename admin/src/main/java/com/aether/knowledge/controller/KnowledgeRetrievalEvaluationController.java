...
public class KnowledgeRetrievalEvaluationController {
    ...
    @Permission(path = "/knowledge/evaluation", type = Permission.Type.Write) @PostMapping("/sets/{id}/runs") public WebResponse<String> createRun(@PathVariable String id, @RequestParam(value="evaluationSetVersionId",required=false) String evaluationSetVersionId) {
        ...
        else { KnowledgeRetrievalEvaluationSetVersion version=setVersionMapper.selectById(evaluationSetVersionId); if(version==null||Boolean.TRUE.equals(version.getDeleted())||!id.equals(version.getEvaluationSetId())) throw new ServerException(404,"Evaluation set version not found."); com.alibaba.fastjson2.JSONObject snapshot=JSON.parseObject(version.getSnapshotJson()); cases=snapshot.getList("cases",KnowledgeRetrievalEvaluationCaseEntity.class); runLabels=snapshot.getList("labels",KnowledgeRetrievalEvaluationLabel.class); datasetSnapshot=version.getSnapshotJson(); }
    ...
    @Permission(path = "/knowledge/evaluation", type = Permission.Type.Write) @PostMapping("/sets/{setId}/cases/{caseId}/labels") public WebResponse<String> saveLabel(@PathVariable String setId, @PathVariable String caseId, @RequestBody KnowledgeRetrievalEvaluationLabel label) {
        requireCase(setId, caseId); String targetType=StringUtils.upperCase(label.getTargetType()); if(!"DOCUMENT".equals(targetType)&&!"SECTION".equals(targetType)&&!"CHUNK".equals(targetType)) throw new ServerException(400,"targetType must be DOCUMENT, SECTION, or CHUNK");
        if(StringUtils.isBlank(label.getDocumentId()) || "SECTION".equals(targetType)&&StringUtils.isBlank(label.getSectionPath()) || "CHUNK".equals(targetType)&&StringUtils.isBlank(label.getChunkId())) throw new ServerException(400,"A document, section, or chunk target is required.");
        ...
        if(!existing.isEmpty()&&!targetType.equals(StringUtils.upperCase(existing.get(0).getTargetType()))) throw new ServerException(409,"All labels for a case must use the same targetType.");
        label.setEvaluationCaseId(caseId); label.setTargetType(targetType); if(label.getRelevanceGrade()==null)label.setRelevanceGrade(1); if(label.getRelevanceGrade()<1||label.getRelevanceGrade()>3)throw new ServerException(400,"relevanceGrade must be between 1 and 3"); if(label.getIsRequired()==null)label.setIsRequired(true); if(label.getStatus()==null)label.setStatus(1); labelMapper.insert(label); return WebResponse.OK(label.getId());
    }
    ...
    @Permission(path = "/knowledge/evaluation", type = Permission.Type.Write) @DeleteMapping("/sets/{id}") public WebResponse<Void> deleteSet(@PathVariable String id) { KnowledgeRetrievalEvaluationSet set=requireSet(id); if(runMapper.selectCount(Wrappers.lambdaQuery(KnowledgeRetrievalEvaluationRun.class).eq(KnowledgeRetrievalEvaluationRun::getEvaluationSetId,id).eq(KnowledgeRetrievalEvaluationRun::getDeleted,false))>0) throw new ServerException(409,"Evaluation sets with run history cannot be deleted."); set.setDeleted(true); setMapper.updateById(set); return WebResponse.OK(I18nUtils.getMessage("knowledge.evaluation.set.delete.success")); }
    /** 查询指定评测集下未删除的评测问题。 */
    ...
    @Permission(path = "/knowledge/evaluation", type = Permission.Type.Write) @PostMapping("/sets/{id}/cases/batch-status") public WebResponse<Void> batchCaseStatus(@PathVariable String id, @RequestBody CaseStatusRequest request) {
        if(request==null||request.getCaseIds()==null||request.getCaseIds().isEmpty()||request.getStatus()==null||request.getStatus()!=0&&request.getStatus()!=1) throw new ServerException(400,"caseIds and status are required.");
    ...
    @Permission(path = "/knowledge/evaluation", type = Permission.Type.Write) @PostMapping("/sets/{id}/cases/import") public WebResponse<Integer> importCases(@PathVariable String id, @RequestBody List<KnowledgeRetrievalEvaluationCaseTransferVo> items) {
        KnowledgeRetrievalEvaluationImportPreviewVo preview=validateImport(requireSet(id), items); if(!preview.isValid()) throw new ServerException(400,"Evaluation case import validation failed.");
    ...
    @Permission(path = "/knowledge/evaluation", type = Permission.Type.Write) @PostMapping("/sets/{setId}/runs/{runId}/baseline") public WebResponse<Void> setBaseline(@PathVariable String setId,@PathVariable String runId) {
        KnowledgeRetrievalEvaluationRun run=requireRun(setId,runId); if(!"SUCCEEDED".equals(run.getStatus())&&!"PARTIAL_FAILED".equals(run.getStatus())) throw new ServerException(409,"Only completed runs can be used as a baseline.");
    ...
    private KnowledgeRetrievalEvaluationCaseEntity requireCase(String setId, String caseId) {
        ...
        if(item==null||Boolean.TRUE.equals(item.getDeleted())||!setId.equals(item.getEvaluationSetId())) throw new ServerException(404,"Evaluation case not found.");
        return item;
    }
    ...
    private void requireHealthy(String setId) {
        ...
        if (!health.isHealthy()) throw new ServerException(409, "Evaluation set has blocking health issues.");
    }
