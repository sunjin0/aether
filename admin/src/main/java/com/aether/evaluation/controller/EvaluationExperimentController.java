...
public class EvaluationExperimentController {
    ...
    public WebResponse<?> detail(@PathVariable String id) {
        EvaluationExperiment experiment=service.getById(id);
    ...
    public WebResponse<?> delete(@PathVariable String id) {
        if (service.getById(id) == null) return WebResponse.Error(404, I18nUtils.getMessage("agent.evaluation.experiment.not-found"));
    ...
    public WebResponse<?> precheck(@RequestBody EvaluationExperiment request) {
        ...
        boolean targetExists="AGENT".equals(request.getTargetType())?agentService.getById(request.getTargetId())!=null:"WORKFLOW".equals(request.getTargetType())&&workflowService.getById(request.getTargetId())!=null;
    ...
    public WebResponse<?> resultDetail(@PathVariable String id,@PathVariable String resultId){EvaluationResult result=resultService.lambdaQuery().eq(EvaluationResult::getId,resultId).eq(EvaluationResult::getExperimentId,id).one();return result==null?WebResponse.Error(404,I18nUtils.getMessage("agent.evaluation.result.not-found")):WebResponse.OK(result);}
    ...
    public WebResponse<?> export(@PathVariable String id) {
        EvaluationExperiment experiment = service.getById(id);
        if (experiment == null) return WebResponse.Error(404, I18nUtils.getMessage("agent.evaluation.experiment.not-found"));
    ...
    public WebResponse<?> review(@PathVariable String id, @PathVariable String resultId, @RequestBody java.util.Map<String, Object> request) {
        ...
        EvaluationExperiment experiment=service.getById(id); if(experiment==null)return WebResponse.Error(404,I18nUtils.getMessage("agent.evaluation.experiment.not-found"));
    ...
    public WebResponse<?> rerunFailed(@PathVariable String id,@RequestBody(required=false) java.util.Map<String,Object> request) {
        if(!evaluationEnabled)return WebResponse.Error(503,I18nUtils.getMessage("agent.evaluation.disabled"));
        EvaluationExperiment source=service.getById(id);if(source==null)return WebResponse.Error(404,I18nUtils.getMessage("agent.evaluation.experiment.not-found"));
    ...
    public WebResponse<?> setBaseline(@RequestBody java.util.Map<String,Object> request) {
        String experimentId=request==null?null:String.valueOf(request.get("experimentId"));EvaluationExperiment experiment=service.getById(experimentId);if(experiment==null)return WebResponse.Error(404,I18nUtils.getMessage("agent.evaluation.experiment.not-found"));
        ...
        String owner=CurrentUser.getUser()==null?null:CurrentUser.getUser().get("userId");EvaluationBaseline existing=baselineService.getOne(com.baomidou.mybatisplus.core.toolkit.Wrappers.lambdaQuery(EvaluationBaseline.class).eq(EvaluationBaseline::getOwnerId,owner).eq(EvaluationBaseline::getTargetType,experiment.getTargetType()).eq(EvaluationBaseline::getTargetId,experiment.getTargetId()).eq(EvaluationBaseline::getDatasetVersionId,experiment.getDatasetVersionId()).eq(EvaluationBaseline::getConfigHash,experiment.getConfigHash()).eq(EvaluationBaseline::getDeleted,false),false);
        if(existing==null){existing=new EvaluationBaseline();existing.setOwnerId(owner);existing.setTargetType(experiment.getTargetType());existing.setTargetId(experiment.getTargetId());existing.setDatasetVersionId(experiment.getDatasetVersionId());existing.setConfigHash(experiment.getConfigHash());existing.setRevision(0L);}existing.setExperimentId(experimentId);existing.setRevision((existing.getRevision()==null?0:existing.getRevision())+1);baselineService.saveOrUpdate(existing);return WebResponse.OK(existing);
    }
    /** Lists the caller's baseline records.  The client uses this to select only a compatible baseline before comparison. */
    @GetMapping("/baselines")
    public WebResponse<List<EvaluationBaseline>> baselines(@RequestParam(required=false) String targetType,@RequestParam(required=false) String targetId,@RequestParam(required=false) String datasetVersionId,@RequestParam(required=false) String configHash) {
        String owner=CurrentUser.getUser()==null?null:CurrentUser.getUser().get("userId");
        return WebResponse.OK(baselineService.lambdaQuery().eq(EvaluationBaseline::getOwnerId,owner).eq(targetType!=null,EvaluationBaseline::getTargetType,targetType).eq(targetId!=null,EvaluationBaseline::getTargetId,targetId).eq(datasetVersionId!=null,EvaluationBaseline::getDatasetVersionId,datasetVersionId).eq(configHash!=null,EvaluationBaseline::getConfigHash,configHash).eq(EvaluationBaseline::getDeleted,false).orderByDesc(EvaluationBaseline::getUpdatedAt).list());
    }
